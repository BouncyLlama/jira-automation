...
pub fn do_get<T: DeserializeOwned, S: Serialize>(req_url: &String, ctx: &Cli, query_params: S) -> Result<T, AppError> {
    ...
    if !res.status().is_success() {
        return Err(AppError::ApiCallBadStatus(format!("status code {}", res.status().as_str())));
    }
    match res.json::<T>() {
        Ok(r) => Ok(r),
...
pub fn do_post<T: DeserializeOwned + 'static, S: Serialize>(req_url: &String, ctx: &Cli, post_body: &S) -> Result<Option<T>, AppError> {
    let client = reqwest::blocking::Client::new();
...
pub fn do_put<T: DeserializeOwned + 'static, S: Serialize>(req_url: &String, ctx: &Cli, put_body: &S) -> Result<Option<T>, AppError> {
    let client = reqwest::blocking::Client::new();
    let token = base64::encode(format!("{}:{}", ctx.user_email, ctx.auth_token));
    let res = client
    ...
    if !status.is_success() {
        return Err(AppError::ApiCallBadStatus(format!("status code {}", status.as_str())));
    }
    if TypeId::of::<T>() == TypeId::of::<()>() {
        Ok(None)
    } else {
...
mod tests {
    ...
    fn test_deser_error() -> Result<(), Box<dyn std::error::Error>> {
        ...
        server.expect(
            Expectation::matching(any()).respond_with(json_encoded("")),
        );
    ...
    fn test_server_error() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let url = server.url("/foo");
        server.expect(
            Expectation::matching(any()).respond_with(status_code(500)),
        );
        let res = do_get::<Release, HashMap<&str, &str>>(&url.to_string(), &ctx, params);
